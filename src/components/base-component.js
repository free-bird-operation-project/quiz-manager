...
class BaseComponent {
	/**
	 * Initializes the class instance with the provided configuration after sanitizing it.
	 * If the configuration is not of type 'object', an empty object is used instead.
	 *
	 * @param {Object} config - The configuration object to be sanitized.
	 */
	constructor(config) {
	...
	_createContainer(tagName, attributes, booleanAttributes) {
		...
		const isBooleanAttributes = sanitizeValue(booleanAttributes, 'object')
		...
		if (isBooleanAttributes) {
			const booleanAttributes = isBooleanAttributes
			Object.entries(booleanAttributes).forEach(([key, value]) => {
